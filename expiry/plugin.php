...
// FIXME api works great, public interface not so much
// No direct call
...
function show_expiry_tablerow($row, $keyword, $url, $title, $ip, $clicks, $timestamp) {
	...
	if($expiry_expose !== "false") {
		...
		$expiry = $ydb->get_row("SELECT * FROM `expiry` WHERE BINARY `keyword` = '$keyword'");
...
function expiry_check( $args ) {
	...
	$expiry = $ydb->get_row("SELECT * FROM `expiry` WHERE `keyword` = '$keyword'");
